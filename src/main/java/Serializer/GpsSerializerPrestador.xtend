...
class GpsSerializerPrestador extends StdSerializer<Servicio> {
	...
	override serialize(Servicio value, JsonGenerator gen, SerializerProvider provider) throws IOException {
		...
		gen.writeStringField("lat", value.latitudPrestador);
		gen.writeStringField("lng", value.longitudPrestador);
		gen.writeEndObject();
	}
	...
	static def mapper() {
		...
		module.addSerializer(Servicio, new GpsSerializerPrestador(Servicio))
